...
import ModalidadeSelector from './ModalidadeSelector.vue';
...
const emit = defineEmits<{
    close: [modalidade: 'nacional' | 'municipal']
}>();
...
function prevPage() {
    if (!isFirstPage.value) {
        currentPage.value--;
    } else if (showModalidade.value) {
        showModalidade.value = false;
    }
}
...
function goToModalidade() {
    showModalidade.value = true;
}

function handleModalidadeSelect(modalidade: 'nacional' | 'municipal') {
    emit('close', modalidade);
}
...
<template>
    <div class="tutorial-container">
        ...
        <!-- TELA DE ESCOLHA DE MODALIDADE -->
        <template v-if="showModalidade">
            <header class="tutorial-header">
                <h3>Quase lá!</h3>
                <button class="btn-skip" @click="showModalidade = false">← Voltar ao tutorial</button>
            </header>

            <div class="tutorial-viewport modalidade-viewport">
                <ModalidadeSelector @select="handleModalidadeSelect" />
            </div>
